...
Future<void> main() async {
  ...
  setupComplete = await getBool("setupComplete");
  ...
  runApp(
    ...
    MultiProvider(
      providers: [
        ChangeNotifierProvider.value(value: globalLoader),
        ChangeNotifierProvider.value(value: eventProviderGlobal),
      ],
      child: const HazelnutApp(),
    ),
    */
  );
}
...
Color getRandomAccentColor() {
  ...
  int min = 100; // minimaler Farbwert
  int max = 255;
  return Color.fromARGB(
    255,
    min + random.nextInt(max - min), // Rot
    min + random.nextInt(max - min), // Grün
    min + random.nextInt(max - min), // Blau
  );
}
